...
# (une blague, un truc qui leur dit que leur dernier


# Démarrage
client = login_twitter

puts "Après la connexion ======================  ! #{client}"


# Je récupère mon propre historique - fil des tweets
mon_fil_tweet = client.home_timeline

mon_fil_tweet.each {|tweet| puts tweet.full_text} # text existe aussi
mon_fil_tweet.each {|tweet| puts tweet.text} # text existe aussi


tweets = client.user_timeline('rubyinside', count: 20)


# 1- Liker (Favorite) les messages avec le #bonjour_monde
# -------------------------------------------------------
...
# Ecrire 2 messages à des personnes qui ont posté le #bonjour_monde
# -----------------------------------------------------------------
client.search('#bonjour_monde').take(2).each do |tweet|
  client.update("@#{tweet.user.screen_name} Hey moi aussi je dis #bonjour_monde !!! @the_hacking_pro :)")
  puts "@#{tweet.user.screen_name}"
end


# Petits tests récup un user depuis son handle @name et récup de son id
client.user("@Ar_no_du_rang") #<Twitter::User id=1280838968318312451>
id1 = client.user("@Ar_no_du_rang")
puts "L'id de est #{id1.id}"
id2 = client.user(id1.id)
puts id2

# Depuis le PRY, on accède à la commande ls Objet (tout est objet), et ça nous donne tous les methodes applicables
# ----------------------------------------------------------------------------------------------------------------

# Qui suit @Ar_no_du_rang ?
# -> Me renvoie vers le curseur

puts client.followers("@Ar_no_du_rang")

suiveurs = client.followers(id1.id)

binding.pry

client.update('#bonjour_monde   !   Hello @the_hacking_pr  !')
